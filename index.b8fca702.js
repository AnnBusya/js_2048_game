...
function handleKeydown(e) {
    ...
    game.updateUI();
    if (game.getStatus() === "win") document.querySelector(".message-win").classList.remove("hidden");
    else if (game.getStatus() === "lose") document.querySelector(".message-lose").classList.remove("hidden");
}
...
class Game {
    ...
    initializeGame() {
        this.buttonStart.addEventListener("click", ()=>{
            if (this.status === "idle" || this.status === "lose" || this.status === "win") this.start();
            else this.restart();
            this.updateUI();
    ...
    moveLeft() {
        ...
        for(let row = 0; row < 4; row++){
            ...
            if (this.state[row].join("") !== newRow.join("")) moved = true;
    ...
    updateUI() {
        ...
        gameCells.forEach((cell, index)=>{
            ...
            cell.className = `field-cell ${cellValue > 0 ? `field-cell--${cellValue}` : ""}`;
        });
        document.querySelector(".game-score").textContent = this.score;
        if (this.status === "win") document.querySelector(".message-win").classList.remove("hidden");
        else if (this.status === "lose") document.querySelector(".message-lose").classList.remove("hidden");
    }
    checkGameStatus() {
    ...
    checkWin() {
        for(let rowIndex = 0; rowIndex < 4; rowIndex++)for(let colIndex = 0; colIndex < 4; colIndex++){
            if (this.state[rowIndex][colIndex] === 2048) return true;
        }
        return false;
    }
    checkLose() {
        for(let r = 0; r < 4; r++)for(let c = 0; c < 4; c++){
            ...
            if (r > 0 && this.state[r][c] === this.state[r - 1][c]) return false;
            if (r < 3 && this.state[r][c] === this.state[r + 1][c]) return false;
            if (c > 0 && this.state[r][c] === this.state[r][c - 1]) return false;
            if (c < 3 && this.state[r][c] === this.state[r][c + 1]) return false;
        }
        return true;
    }
}
